import React from "react";
 ...
 const UserData =[
    ...
    function FindNearestLocation(lat1, lon1, lat2, lon2) {
        lat1 = Deg2Rad(lat1);
        lat2 = Deg2Rad(lat2);
        lon1 = Deg2Rad(lon1);
        lon2 = Deg2Rad(lon2);
    ...
    for(var i = 0; i < UserData.length; i++) {
        ...
        var check=Math.floor(parseInt(FindNearestLocation(lat1,lon1,obj.latitude,obj.longitude)));
        if(check < 100){
            console.log(check);
            SortedNearList.push([ obj.user_id ,obj.name,check]);
            
        }
    }
  var result=  SortedNearList.sort(function(a,b) {
        return a[0] - b[0]});
